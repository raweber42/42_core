...
/*   Updated: 2022/05/07 14:33:54 by raweber          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// a static buffer array called temp. HERE IT IS AN TWO-DIMENSIONAL
// ARRAY WHICH STORES DIFFERENT 'TEMP's FOR DIFFERENT FILE DESCRIPTORS 
// AT THE FILE DESCRIPTOR'S INDEX. If there is data left in temp
...
char	*get_next_line(int fd)
{
	char			*result;
	int				bytes_read;
	static char		temp[10240][BUFFER_SIZE + 1];

	if (fd < 0 || fd > 10240 || BUFFER_SIZE <= 0)
		return (NULL);
	result = NULL;
	if (temp[fd][0] != 0)
		result = ft_strdup(temp[fd]);
	temp[fd][BUFFER_SIZE] = '\0';
	while (!ft_strchr(result, '\n'))
	{
		bytes_read = read(fd, temp[fd], BUFFER_SIZE);
		if (!bytes_read)
			break ;
		else if (bytes_read == -1)
			return (NULL);
		if (bytes_read < BUFFER_SIZE)
			temp[fd][bytes_read] = '\0';
		result = update_result(result, temp[fd]);
	}
	edit_temp(temp[fd]);
	if (!bytes_read && !temp[fd][0] && !result)
		return (NULL);
